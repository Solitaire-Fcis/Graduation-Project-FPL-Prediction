...
def select(gw, data_in, print_output=False):
    ...
    if (print_output):
        ...
        print('Cost                ' + '£' + str(round(total_cost, 2)) + 'M\n')
    '''
    return predicted_points_total, real_points_total, total_cost, first_team, captain, subs, cal_points
...
def select_team(player_data, budget, sub_factor):
    ...
    for team_id in np.unique(team_codes):
        model += sum(decisions[i] + sub_decisions[i]
                     for i in range(num_players) if player_data.iloc[i].opponent_team == team_id) <= 3  # max 3 players
...
predicted_score, real_score, error, teams_list, captains_list, subs_list, cal_points_list = select(29,Players_Data,True)
print(teams_list, subs_list, cal_points_list)